...
{
    // std::cout << "nfeiehfuhef";
    NS_LOG_FUNCTION(this);
    // std::cout << "ruehrueh" << m_subChannelsForTransmission[0] << std::endl;
...
LteUePhy::GetSubChannelsForReception()
{
    // std::cout << "nfeiehfuhef";
    NS_LOG_FUNCTION(this);
    // std::cout << "ruehrueh" << m_subChannelsForReception[0] << std::endl;
...
LteUePhy::CreateDlCqiFeedbackMessage(const SpectrumValue& sinr)
{
    ...
    if (Simulator::Now() > m_p10CqiLast + m_p10CqiPeriodicity)
    {
        ...
        // cout << "________Цикл по nbSubChannels начался" << endl;
        for (std::size_t i = 0; i < nbSubChannels; i++)
        {
            // cout << "Количество абонентов: " << m_rnti << endl;
            // cout << "______nbSubChannel: " << cqi.at(i) << "____________" << endl;
            if (cqi.at(i) != -1)
            {
                cqiSum += cqi.at(i);
                activeSubChannels++;
...
LteUePhy::ReceiveLteControlMessageList(std::list<Ptr<LteControlMessage>> msgList)
{
    ...
    {
        // auto iter = m_subChannelsForTransmissionQueue.begin();
        // cout << "msg: " << *it << endl;
        // cout << "Queue: " << m_subChannelsForTransmissionQueue.size() << endl;
        // for (iter; iter != m_subChannelsForTransmissionQueue.end(); iter++) {
        //     auto iter1 = (*iter).begin();
        //     for (iter1; iter1 != (*iter).end(); iter1++) {
        //         std::cout << "\n __________Resource Block-Reception: " << *iter1 << "  __________________" << std::endl;
        //     }
        // }
        // for (auto RB : m_subChannelsForTransmission) {
        //     std::cout << "\n __________Resource Block-Transmission: " << RB << "  __________________" << std::endl;
        // }
